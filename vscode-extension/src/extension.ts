...
let pendingCompletionTimeout: NodeJS.Timeout | undefined;
let currentSource: 'claude-code' | 'codex' | null = null;
let lastTerminalSource: 'claude-code' | 'codex' | null = null;
let lastTerminalActivity = 0;
let outputChannel: vscode.OutputChannel;
let logWatcher: fs.FSWatcher | null = null;
let logFilePosition = 0;
let logFilePath: string | null = null;
let logWatcherRetry: NodeJS.Timeout | undefined;
...
let claudeSsePollInterval: NodeJS.Timeout | undefined;
let claudeSseSeenIds = new Set<string>();
const MAX_PENDING_HUB_EVENTS = 50;
const HUB_ERROR_LOG_COOLDOWN_MS = 5000;
const HUB_BACKOFF_MAX_MS = 60000;
let pendingHubEvents: Array<{ source: 'claude-code' | 'codex'; status: string; duration?: number; ts: number }> = [];
...
let claudeDebugWatcher: fs.FSWatcher | null = null;
let claudeDebugDir: string | null = null;
let claudeHistoryWatcher: fs.FSWatcher | null = null;
let claudeHistoryFile: string | null = null;
let claudeHistoryPosition = 0;
let claudeHistoryStartTime = 0;
let claudeHistoryCompletionTimeout: NodeJS.Timeout | null = null;
let claudeHistoryPollInterval: NodeJS.Timeout | null = null;
let claudeProjectWatcher: fs.FSWatcher | null = null;
let claudeProjectDir: string | null = null;
let claudeProjectWatcherStartedAt = 0;
const claudeProjectStates = new Map<string, { position: number; buffer: string }>();
let claudeProjectPollInterval: NodeJS.Timeout | null = null;
let claudeProjectScanInterval: NodeJS.Timeout | null = null;
let debugFilePollInterval: NodeJS.Timeout | undefined;
type DebugFileState = {
    position: number;
...
let debugFileStates: Map<string, DebugFileState> = new Map(); // Track state per file
...
let codexSessionsDir: string | null = null;
let codexSessionsPollInterval: NodeJS.Timeout | undefined;
let codexSessionsWatcher: fs.FSWatcher | null = null;
let codexSessionStates: Map<string, DebugFileState> = new Map();
let codexWatcherStartedAt = 0;

// Configuration
const HUB_URL = () => vscode.workspace.getConfiguration('llmNotify').get<string>('hubUrl') || 'http://localhost:3847';
const isEnabled = () => vscode.workspace.getConfiguration('llmNotify').get<boolean>('enabled') ?? true;
const showVSCodeNotification = () => vscode.workspace.getConfiguration('llmNotify').get<boolean>('showVSCodeNotification') ?? true;
const detectClaudeCode = () => vscode.workspace.getConfiguration('llmNotify').get<boolean>('detectClaudeCode') ?? true;
const detectCodex = () => vscode.workspace.getConfiguration('llmNotify').get<boolean>('detectCodex') ?? true;
const claudePromptPattern = () => vscode.workspace.getConfiguration('llmNotify').get<string>('claudePromptPattern') || '❯\\s';
const claudeOutputPattern = () => vscode.workspace.getConfiguration('llmNotify').get<string>('claudeOutputPattern') || '(⏺|•|·)\\s';
const claudeCompletePattern = () => vscode.workspace.getConfiguration('llmNotify').get<string>('claudeCompletePattern') || '✻\\s+Worked\\s+for';
const debugTerminalData = () => vscode.workspace.getConfiguration('llmNotify').get<boolean>('debugTerminalData') ?? false;
const claudeLogPath = () => vscode.workspace.getConfiguration('llmNotify').get<string>('claudeLogPath') || '';
const claudeSsePort = () => vscode.workspace.getConfiguration('llmNotify').get<number>('claudeSsePort') || 0;
const codexSessionsPath = () => vscode.workspace.getConfiguration('llmNotify').get<string>('codexSessionsDir') || '';
...
export function activate(context: vscode.ExtensionContext) {
    ...
    context.subscriptions.push(
        vscode.commands.registerCommand('llm-notify.testNotification', testNotification),
        vscode.commands.registerCommand('llm-notify.openDashboard', openDashboard),
        vscode.commands.registerCommand('llm-notify.toggleEnabled', toggleEnabled),
        vscode.commands.registerCommand('llm-notify.dismissAlarm', dismissAlarmCommand)
    );
...
function setupTerminalMonitoring(context: vscode.ExtensionContext) {
    ...
    const startExecEvent = (vscode.window as unknown as { onDidStartTerminalShellExecution?: Function }).onDidStartTerminalShellExecution;
    if (startExecEvent) {
        log('Terminal shell execution start event available');
        context.subscriptions.push(
            startExecEvent((event: { terminal: vscode.Terminal; execution?: { commandLine?: string } }) => {
    ...
    const endExecEvent = (vscode.window as unknown as { onDidEndTerminalShellExecution?: Function }).onDidEndTerminalShellExecution;
    if (endExecEvent) {
        log('Terminal shell execution end event available');
        context.subscriptions.push(
            endExecEvent((event: { terminal: vscode.Terminal; execution?: { commandLine?: string } }) => {
                const source = getExecutionSource(event.execution?.commandLine) || getTerminalSource(event.terminal);
...
function setupLogWatcher(context: vscode.ExtensionContext) {
...
function setupClaudeSseWatcher(context: vscode.ExtensionContext) {
...
function fetchClaudeSseOnce(url: string) {
    http.get(url, (res) => {
        ...
        res.on('end', () => {
            ...
            for (const raw of events) {
                ...
                if (!payload) continue;
                let parsed: { id?: string; type?: string } | null = null;
                try {
                ...
                if (claudeSseSeenIds.has(eventId)) continue;
...
function setupClaudeDebugWatcher(context: vscode.ExtensionContext) {
...
function startClaudeDebugWatcher() {
    ...
    fs.promises.access(claudeDebugDir, fs.constants.R_OK)
        .then(() => {
            ...
            try {
                const dirToWatch = claudeDebugDir as string;
                claudeDebugWatcher = fs.watch(dirToWatch, { persistent: false }, (eventType: string, filename: string | null) => {
                ...
                log(`Could not watch debug directory: ${(err as Error).message}`);
            }
...
function setupClaudeHistoryWatcher(context: vscode.ExtensionContext) {
...
function startClaudeHistoryWatcher() {
    if (!claudeHistoryFile || !detectClaudeCode()) return;
    ...
    fs.promises.stat(claudeHistoryFile)
        .then((stat) => {
            ...
            try {
                claudeHistoryWatcher = fs.watch(claudeHistoryFile as string, { persistent: false }, (eventType: string) => {
                    if (eventType === 'change') {
                        readClaudeHistoryUpdates();
                    }
                });
            } catch (err) {
                log(`Could not watch Claude history: ${(err as Error).message}`);
            }
        })
        .catch(() => {
...
async function readClaudeHistoryUpdates() {
    ...
    try {
        ...
        if (stat.size <= claudeHistoryPosition) return;
...
function processClaudeHistoryContent(content: string) {
    ...
    for (const line of lines) {
        ...
        if (ts < claudeHistoryStartTime) continue;
...
async function findAndWatchLatestDebugFile() {
    ...
    try {
        ...
        const validFiles = fileStats.filter((f): f is { file: string; mtime: number; size: number } => f !== null);
        ...
        log(`Error finding debug files: ${(err as Error).message}`);
    }
}
...
function startDebugFilePoll() {
    ...
    debugFilePollInterval = setInterval(async () => {
        if (debugFileStates.size === 0) return;
...
function anyDebugFileGenerating(): boolean {
...
function setupCodexSessionsWatcher(context: vscode.ExtensionContext) {
...
function startCodexSessionsWatcher() {
    ...
    fs.promises.access(codexSessionsDir, fs.constants.R_OK)
        .then(() => {
            ...
            try {
                const dirToWatch = codexSessionsDir as string;
                codexSessionsWatcher = fs.watch(dirToWatch, { persistent: false }, (eventType: string, filename: string | null) => {
                ...
                log(`Could not watch Codex sessions directory: ${(err as Error).message}`);
            }
...
async function findAndWatchCodexSessionFiles() {
    ...
    try {
        ...
        log(`Error finding Codex session files: ${(err as Error).message}`);
    }
}
...
function startCodexSessionsPoll() {
    ...
    codexSessionsPollInterval = setInterval(async () => {
        if (codexSessionStates.size === 0) return;
...
function anyCodexSessionGenerating(): boolean {
...
function hasReliableDetectors(): boolean {
    return Boolean(logFilePath || debugFileStates.size > 0 || codexSessionStates.size > 0);
}

function setCurrentSource(source: 'claude-code' | 'codex') {
    currentSource = source;
    if (isGenerating) {
...
function resolveActiveSource(): 'claude-code' | 'codex' {
    if (anyCodexSessionGenerating()) return 'codex';
    if (anyDebugFileGenerating()) return 'claude-code';
    return currentSource || resolveCurrentSource();
}

function processCodexSessionContent(filePath: string, content: string, minTimestamp?: number) {
    ...
    for (const line of lines) {
        ...
        if (isUserResponseItem) {
            ...
            if (
                lowerMessage.startsWith('# agents.md instructions') ||
        ...
        const isAssistantMessage =
            (entry?.type === 'event_msg' && entry?.payload?.type === 'agent_message' && !!entry?.payload?.message) ||
            (entry?.type === 'response_item' && entry?.payload?.role === 'assistant' && Array.isArray(entry?.payload?.content));

        const isCancelEvent =
            entryType.includes('turn_aborted') ||
...
function processDebugLogContent(filePath: string, content: string) {
...
function setupClaudeProjectsWatcher(context: vscode.ExtensionContext) {
...
function startClaudeProjectsWatcher() {
    if (!claudeProjectDir || !detectClaudeCode()) return;

    claudeProjectWatcherStartedAt = Date.now();
    fs.promises.access(claudeProjectDir, fs.constants.R_OK)
        .then(() => {
            try {
                claudeProjectWatcher = fs.watch(claudeProjectDir as string, { persistent: false }, (eventType: string, filename: string | null) => {
                    if (eventType === 'rename' || eventType === 'change') {
                        findAndWatchClaudeProjectFiles();
                    }
                });
            } catch (err) {
                log(`Could not watch Claude projects: ${(err as Error).message}`);
            }
            findAndWatchClaudeProjectFiles();
            if (!claudeProjectScanInterval) {
...
async function findAndWatchClaudeProjectFiles() {
    ...
    try {
        ...
        for (const file of files) {
            if (claudeProjectStates.has(file)) continue;
            const rel = path.relative(root, file);
            if (rel.includes(`${path.sep}subagents${path.sep}`)) continue;
            if (rel.split(path.sep).length !== 2) continue; // only project/session.jsonl
            let stat: fs.Stats;
            try {
            ...
            if (stat.mtime.getTime() < tenMinutesAgo) continue;
...
function extractClaudeProjectUserText(entry: any): string {
...
function processClaudeProjectContent(state: { position: number; buffer: string }, content: string) {
    const text = state.buffer + content;
    const parts = text.split('\n');
    if (text.endsWith('\n')) {
        state.buffer = '';
    ...
    for (const line of lines) {
        ...
        if (type === 'user' && entry?.message?.role === 'user') {
            ...
            const hasTextContent = Array.isArray(messageContent) &&
                messageContent.some((c: any) => c?.type === 'text' && c?.text?.trim());
...
function tryInitializeLogWatcher() {
    ...
    fs.promises.stat(logFilePath).then((stat) => {
        ...
        const watchPath = logFilePath as string;
        logWatcher = fs.watch(watchPath, { persistent: true }, (eventType: string) => {
...
async function readLogFileAppend() {
    ...
    try {
        ...
        log(`Log watcher read error: ${(error as Error).message}`);
    }
}
...
function isClaudeCodeTerminal(terminal: vscode.Terminal): boolean {
    if (!detectClaudeCode()) return false;
...
function isCodexTerminal(terminal: vscode.Terminal): boolean {
    if (!detectCodex()) return false;
    const name = terminal.name.toLowerCase();
...
function getTerminalSource(terminal: vscode.Terminal): 'claude-code' | 'codex' | null {
    if (isCodexTerminal(terminal)) return 'codex';
    if (isClaudeCodeTerminal(terminal)) return 'claude-code';
    return null;
}
...
function markGenerating(sourceOverride?: 'claude-code' | 'codex'): boolean {
...
function handleTerminalData(data: string, source: 'claude-code' | 'codex') {
    if (!isEnabled()) return;
...
function processClaudeStream(buffer: string) {
    ...
    if (isGenerating && isEnabled()) {
        ...
        if (showVSCodeNotification()) {
            vscode.window.showInformationMessage(
                `${getSourceLabel(currentSource)} has finished generating`,
                'Open Dashboard',
                'Dismiss'
            ).then(selection => {
...
function stopGeneratingSilently(source: 'claude-code' | 'codex') {
...
function updateStatusBar(status: 'idle' | 'generating' | 'complete' | 'disconnected') {
    if (!isEnabled()) {
...
function enqueueHubEvent(source: 'claude-code' | 'codex', status: string, duration?: number) {
    const now = Date.now();
...
async function reportToHub(source: 'claude-code' | 'codex', status: string, duration?: number) {
    if (!isEnabled()) return;

    const hubUrl = HUB_URL();
    const now = Date.now();
    if (!hubConnected || now < nextHubAttemptAt) return;
...
async function flushPendingHubEvents() {
    if (!hubConnected || pendingHubEvents.length === 0) return;
    if (Date.now() < nextHubAttemptAt) return;
    const queue = [...pendingHubEvents];
    pendingHubEvents = [];
    for (const evt of queue) {
        await reportToHub(evt.source, evt.status as any, evt.duration);
    }
}
...
async function testNotification() {
    ...
    vscode.window.showInformationMessage(
        'LLM Notify: Test notification sent!',
        'Open Dashboard'
    ).then(selection => {
        if (selection === 'Open Dashboard') {
            openDashboard();
...
async function toggleEnabled() {
    ...
    vscode.window.showInformationMessage(
        `LLM Notify: ${!currentValue ? 'Enabled' : 'Disabled'}`
    );
...
function resolveCurrentSource(): 'claude-code' | 'codex' {
...
function getSourceLabel(source: 'claude-code' | 'codex' | null): string {
    return source === 'codex' ? 'Codex' : 'Claude Code';
}

function getExecutionSource(commandLine?: string): 'claude-code' | 'codex' | null {
...
function detectSourceFromData(data: string): 'claude-code' | 'codex' | null {
    const clean = stripAnsi(data);
    const outputRegex = new RegExp(claudeOutputPattern());
    const promptRegex = new RegExp(claudePromptPattern());
    const completeRegex = new RegExp(claudeCompletePattern());
    if (
        outputRegex.test(clean) ||
